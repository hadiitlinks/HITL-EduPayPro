...
import { 
  ...
  FileSpreadsheet, Edit3, Trash2, Image, School, Users, CheckSquare, Square, Eye, EyeOff, UploadCloud, Printer, Maximize
} from 'lucide-react';
        ...
        <div className="space-y-4 animate-in slide-in-from-bottom-5 duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {students.filter(s => {
              ...
              <div key={`student-${s.id}`} className="bg-slate-900/80 border border-white/5 rounded-3xl p-4 relative group hover:border-blue-500/50 transition-all">
          ...
          <div className="w-full md:w-80 bg-slate-900 border-r border-white/10 p-5 overflow-y-auto custom-scroll">
             ...
             <div className="space-y-4 mb-6">
               ...
               <div className="p-4 bg-blue-600/5 rounded-2xl border border-blue-500/20 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setCardSize({width: 86, height: 54})} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 rounded text-[8px] font-black text-slate-300 uppercase">Standard</button>
                    <button onClick={() => setCardSize({width: 100, height: 70})} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 rounded text-[8px] font-black text-slate-300 uppercase">Large</button>
                  </div>
               </div>

               <div className="p-3 bg-black/40 rounded-xl border border-white/5">
                  <label className="text-[9px] font-black text-slate-500 uppercase">2. Background Template</label>
               ...
               {['schoolName', 'schoolLogo', 'name', 'roll', 'class', 'father', 'address', 'photo'].map(field => (
                  <div key={`field-${field}`} className="p-3 bg-black/20 rounded-xl border border-white/5 space-y-2">
