...
import { 
  ...
  FileSpreadsheet, Edit3, Trash2, Image, School, Users, CheckSquare, Square, Eye, EyeOff, UploadCloud, Printer, Maximize, CheckCircle2
} from 'lucide-react';
...
const StudentRegistration = () => {
  ...
  const fetchData = async () => {
    ...
    setClasses(classList.sort((a, b) => a.className.localeCompare(b.className, undefined, {numeric: true, sensitivity: 'base'})));
  ...
  const exportFilteredStudents = () => {
    ...
    const data = filtered.map(s => ({
        "Reg No": s.regNo, "Roll No": s.rollNo, "Full Name": s.fullName,
        "Father Name": s.fatherName, "Class": s.class, "Phone": s.parentPhone,
        "Address": s.address, "Fee": s.monthlyFee
    }));
    ...
    reader.onload = async (evt) => {
      ...
      json.forEach((row, index) => {
        ...
        batch.set(newDoc, {
          ...
          class: row["Class"] || "", rollNo: row["Roll No"] || "",
          parentPhone: row["Parent Phone"] || "", address: row["Address"] || "",
          dob: row["DOB"] || "", serialNo: (students.length + index + 1).toString(),
  ...
  const getFilteredStudents = () => {
    return students.filter(s => {
      const matchesSearch = (s.fullName || "").toLowerCase().includes(listSearch.toLowerCase());
  ...
  const toggleSelectAll = () => {
    const filtered = getFilteredStudents();
    if (selectedBatch.length === filtered.length) {
      setSelectedBatch([]);
    } else {
      setSelectedBatch(filtered.map(s => s.id));
    }
  };
  ...
  const IDCardComponent = ({ student }) => (
    <div style={{ width: `${cardSize.width}mm`, height: `${cardSize.height}mm`, position: 'relative', overflow: 'hidden', backgroundColor: '#fff' }} className="shadow-2xl rounded-xl print:shadow-none print:rounded-none">
      ...
      <div className="relative z-10 w-full h-full font-sans">
        ...
        {visibility.photo && (
          <div style={{ position: 'absolute', top: positions.photo.top, left: positions.photo.left, width: positions.photo.size, height: positions.photo.size }} className="border border-black/10 overflow-hidden bg-slate-100">
            {student.photo ? <img src={student.photo} className="w-full h-full object-cover" alt="Student" /> : <User className="w-full h-full p-4 text-slate-300" />}
          </div>
        )}
        {visibility.name && <div style={{ position: 'absolute', top: positions.name.top, left: positions.name.left, fontSize: positions.name.size, color: positions.name.color }} className="font-black uppercase">{student.fullName}</div>}
        {visibility.roll && <div style={{ position: 'absolute', top: positions.roll.top, left: positions.roll.left, fontSize: positions.roll.size, color: positions.roll.color }} className="font-bold">Roll: {student.rollNo}</div>}
        {visibility.class && <div style={{ position: 'absolute', top: positions.class.top, left: positions.class.left, fontSize: positions.class.size, color: positions.class.color }} className="font-bold">Class: {student.class}</div>}
        {visibility.father && <div style={{ position: 'absolute', top: positions.father.top, left: positions.father.left, fontSize: positions.father.size, color: positions.father.color }} className="font-bold uppercase">Father: {student.fatherName}</div>}
        {visibility.address && <div style={{ position: 'absolute', top: positions.address.top, left: positions.address.left, fontSize: positions.address.size, color: positions.address.color }} className="font-medium">{student.address}</div>}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 p-3 md:p-6 font-sans">
      ...
      {/* Fixed Hidden Print Section with Unique Keys */}
      <div className="hidden print:block print:bg-white">
        <div className="flex flex-wrap gap-4 p-4 justify-center">
          {students.filter(s => selectedBatch.includes(s.id)).map((student, idx) => (
            <div key={`print-card-idx-${student.id}-${idx}`} className="print-card-wrapper">
              <IDCardComponent student={student} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="print:hidden">
        ...
        {activeTab === 'registration' ? (
                ...
                <div className="space-y-1 relative">
                  ...
                  {showDropdown && (
                    <div className="absolute z-50 w-full mt-1 bg-slate-900 border border-white/10 rounded-xl max-h-40 overflow-auto">
                      {classes.map(c => <div key={`cls-${c.id}`} onClick={() => { setFormData({...formData, class: c.className, originalFee: c.monthlyFee, monthlyFee: c.monthlyFee - formData.discount}); setSearchTerm(c.className); setShowDropdown(false); }} className="p-3 text-[10px] font-black uppercase text-white hover:bg-blue-600 cursor-pointer">{c.className}</div>)}
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-4 animate-in slide-in-from-bottom-5 duration-300">
            <div className="flex flex-col md:flex-row gap-3 bg-slate-900/60 p-3 rounded-2xl border border-white/5 items-center">
              <div className="flex-1 flex items-center bg-slate-950 px-4 py-3 rounded-xl border border-white/10 gap-3 w-full">
              ...
              <select value={filterClass} onChange={e => { setFilterClass(e.target.value); setSelectedBatch([]); }} className="w-full md:w-48 bg-slate-950 text-[10px] font-black text-white p-3 rounded-xl border border-white/10 uppercase">
                <option value="All">All Classes</option>
                {classes.map(c => <option key={`opt-${c.id}`} value={c.className}>{c.className}</option>)}
              </select>
              
              {/* Select All / Deselect All Logic */}
              {getFilteredStudents().length > 0 && selectedBatch.length > 0 && (
                <button 
                  onClick={toggleSelectAll} 
                  className={`w-full md:w-auto px-6 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all border ${
                    selectedBatch.length === getFilteredStudents().length 
                    ? 'bg-red-500/10 text-red-400 border-red-500/30' 
                    : 'bg-blue-600 text-white border-blue-500'
                  }`}
                >
                  {selectedBatch.length === getFilteredStudents().length ? <><X size={16}/> Deselect All</> : <><CheckCircle2 size={16}/> Select All</>}
                </button>
              )}

              <button onClick={exportFilteredStudents} className="w-full md:w-auto bg-emerald-600 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-emerald-500">
            ...
            {selectedBatch.length > 0 && (
              <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-blue-600 p-3 rounded-2xl shadow-2xl flex items-center gap-4 z-[100] border border-white/20">
                ...
                <button onClick={handlePrintBatch} className="p-2.5 bg-emerald-500 rounded-xl flex items-center gap-2 text-[10px] font-black text-white"><Printer size={16}/> PRINT CARDS</button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {getFilteredStudents().map((s) => (
                <div key={`ui-student-${s.id}`} className={`bg-slate-900/80 border rounded-3xl p-4 relative group transition-all ${selectedBatch.includes(s.id) ? 'border-blue-500 bg-blue-500/5' : 'border-white/5 hover:border-blue-500/50'}`}>
                  <button onClick={() => toggleSelect(s.id)} className="absolute top-4 right-4 z-10">
                     {selectedBatch.includes(s.id) ? <CheckSquare size={20} className="text-blue-500 animate-in zoom-in duration-200"/> : <Square size={20} className="text-slate-700"/>}
                  </button>
                  <div className="flex items-center gap-4">
                  ...
                  <div className="mt-4 flex gap-2">
                     <button onClick={() => { setFormData({...s}); setIsEditing(true); setActiveTab('registration'); }} className="flex-1 bg-blue-600/10 hover:bg-blue-600 text-blue-400 hover:text-white p-2 rounded-xl transition-all flex items-center justify-center gap-2 text-[10px] font-black uppercase">
                       <Edit3 size={14}/> Edit
                     </button>
                     <button onClick={() => { setSelectedStudent(s); setShowID(true); }} className="flex-1 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-400 hover:text-white p-2 rounded-xl transition-all flex items-center justify-center gap-2 text-[10px] font-black uppercase">
                       <CreditCard size={14}/> ID Card
                     </button>
                     <button onClick={async () => { if(window.confirm('Delete student?')) { await deleteDoc(doc(db, "students", s.id)); fetchData(); } }} className="bg-red-600/10 hover:bg-red-600 text-red-400 hover:text-white p-2 rounded-xl transition-all">
                       <Trash2 size={14}/>
                     </button>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-[200] bg-[#020617] flex flex-col md:flex-row overflow-hidden">
            <div className="w-full md:w-80 bg-slate-900 border-r border-white/10 p-5 overflow-y-auto custom-scroll">
               ...
               <div className="space-y-4 mb-6">
                 <div className="p-4 bg-blue-600/5 rounded-2xl border border-blue-500/20 space-y-3">
                    <div className="flex items-center gap-2 mb-2">
                      <Maximize size={16} className="text-blue-400"/>
                      <span className="text-[10px] font-black text-white uppercase italic">Card Dimensions (mm)</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <p className="text-[8px] font-bold text-slate-500 uppercase">Width</p>
                        <input type="number" value={cardSize.width} onChange={e => setCardSize({...cardSize, width: parseInt(e.target.value) || 0})} className="w-full bg-slate-950 p-2.5 rounded-lg text-white font-bold text-xs border border-white/10" />
                      </div>
                      <div className="space-y-1">
                        <p className="text-[8px] font-bold text-slate-500 uppercase">Height</p>
                        <input type="number" value={cardSize.height} onChange={e => setCardSize({...cardSize, height: parseInt(e.target.value) || 0})} className="w-full bg-slate-950 p-2.5 rounded-lg text-white font-bold text-xs border border-white/10" />
                      </div>
                    </div>
                 </div>
                 <div className="p-3 bg-black/40 rounded-xl border border-white/5">
                    <label className="text-[9px] font-black text-slate-500 uppercase">Background Template</label>
                    <label className="flex items-center justify-center gap-2 p-3 bg-blue-600/10 border border-blue-500/20 text-blue-400 rounded-xl mt-2 cursor-pointer">
                      <Image size={16}/> <span className="text-[9px] font-black">Upload Background</span>
                 ...
                 {['schoolName', 'schoolLogo', 'name', 'roll', 'class', 'father', 'address', 'photo'].map(field => (
                    <div key={`studio-field-${field}`} className="p-3 bg-black/20 rounded-xl border border-white/5 space-y-2">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 bg-slate-950 relative">
                <div ref={cardRef}>
                  <IDCardComponent student={selectedStudent} />
                </div>
